...
export default function Object({ factor, speed, xFactor, yFactor, zFactor }) {
    ...
    useFrame((state) => {
        ...
        let y = Math.sin(timeSpeedFactor) + Math.cos(timeSpeedFactor * 2) / 10 + yFactor + Math.sin((timeSpeedFactor / 10) * factor) + (Math.cos(timeSpeedFactor * 2) * factor) / 10
        y = Math.min(y, 5) // Clamp to max of 7
